...
library(rootSolve)
path.dat <- "/nas/longleaf/home/haolin/adane/s10/data/"
path.results <- "/nas/longleaf/home/haolin/adane/s10/results/"
...
for (j in 1:nsim){
  ...
  # check ratios
  ratio[j] = table(dat[(is.na(dat$Z1)==F),]$delta.epsilon)[2]/(nrow(dat[(is.na(dat$Z1)==F),])-table(dat[(is.na(dat$Z1)==F),]$delta.epsilon)[2])
